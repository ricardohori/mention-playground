    ...
    AutoCompleteInput.prototype = {
        ...
        renderInput: function () {
            var rawHtml =  '<span id="autocomplete">' +
                '<span id="autocomplete-delimiter">' + this.options.delimiter[this.options.delimiter.length - 1] + '</span>' +
        ...
        init: function (ed) {
            ...
            function prevCharIsSpace(text) {
                if(text.length >= (autoCompleteData.delimiter[0].length + 1)){
                    var lastCharBeforeDelimiter = getPreviousChars(text, autoCompleteData.delimiter[0].length + 1)[0];
                    return (!$.trim(lastCharBeforeDelimiter).length);
                }else{
                    return true;
                }
            }
            ...
            function getCurrentText(e){
                var currentText = ed.selection.getRng(true).startContainer.data || '';
                return currentText + String.fromCharCode(e.which || e.keyCode);
            }

            function getPreviousChars(text, length){
